...
```{r Histogram}
nyc_rest %>% 
  plot_ly(x = ~score, type = "histogram", color = ~boro, colors = "viridis")
...
nyc_rest %>% 
  ...
  mutate(year = fct_reorder(violation_code, n)) %>% 
  plot_ly(x = ~violation_code, y = ~n, type = "bar", text = ~violation_description) 
...
nyc_rest %>%
 ...
 nyc_rest %>% 
  plot_ly(x = ~cuisine_description, type = "histogram", color = ~boro)
...
nyc_rest %>% 
  ...
  plot_ly(
     x = ~inspection_date, y = ~score, type = "scatter", mode = "markers", 
    text = ~violation_description, alpha = 0.5, color = ~critical_flag, colors = "viridis")
```
